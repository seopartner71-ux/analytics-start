...
import { ReportView, type ReportData, type ReportModule } from "./ReportView";
import { generateDailyVisits, generatePagesData, computeKpis, type DateRange } from "@/lib/data-generators";
...
export default function ReportPreviewPage({ isPublic }: Props) {
  ...
  const modules: ReportModule[] = useMemo(() => {
    ...
    if (template?.modules && Array.isArray(template.modules)) {
      return (template.modules as any[]).map((m: any) => ({
        key: m.key,
        enabled: m.enabled ?? true,
        comment: m.comment,
      }));
    }
    return ["kpi", "traffic", "sources", "seo", "pages", "worklog", "ai"].map((key) => ({
      key, enabled: true,
    }));
  ...
  const reportData: ReportData | null = useMemo(() => {
    ...
    if (metrikaStats?.visits_by_day && Array.isArray(metrikaStats.visits_by_day)) {
      trafficChart = (metrikaStats.visits_by_day as any[]).map((d: any) => ({
        ...
        search: Math.round((d.visits || d.search || 0) * 0.45),
        direct: Math.round((d.visits || d.direct || 0) * 0.25),
        social: Math.round((d.visits || d.social || 0) * 0.18),
        referral: Math.round((d.visits || d.referral || 0) * 0.12),
      }));
    } else {
      ...
      trafficChart = daily.map((d) => ({
        ...
        social: Math.round(d.visits * 0.18),
        referral: Math.round(d.visits * 0.12),
      }));
    }

    // KPIs
    const totalVisits = metrikaStats?.total_visits || trafficChart.reduce((s, d) => s + d.search + d.direct + d.social + d.referral, 0);
    ...
    if (sources && Array.isArray(sources) && sources.length > 0) {
      ...
      sourcesChart = [
        ...
        { name: "Социальные", value: Math.round(totalVisits * 0.18) },
        { name: "Реферальные", value: Math.round(totalVisits * 0.12) },
      ];
    }
  ...
  }, [project, metrikaStats, workLogs, cachedReport, template, periodParam]);
