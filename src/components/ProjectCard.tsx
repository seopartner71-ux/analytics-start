...
function Sparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-6" fill="none" preserveAspectRatio="none">
      ...
      <polyline points={points} stroke="hsl(var(--success))" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
...
function DeltaBadge({ value, invert }: { value: number; invert?: boolean }) {
  ...
  const display = invert ? (value < 0 ? `${value}` : `+${value}`) : (value > 0 ? `+${value}` : `${value}`);
  return (
    <Badge
      variant="secondary"
...
export function ProjectCard({
  name, url, initials, color, logoUrl, description,
  seoSpecialist, accountManager, stats, onClick,
}: ProjectCardProps) {
        ...
        <CardContent className="p-3 space-y-2.5">
          ...
          <div className="flex items-start gap-2.5">
            <Tooltip>
              ...
              <TooltipContent side="bottom" className={TOOLTIP_STYLE}>Перейти к детальному отчету проекта</TooltipContent>
            </Tooltip>
            ...
            <div className="flex-1 min-w-0">
              ...
              {domain && (
                <Tooltip>
                  ...
                  <TooltipContent side="bottom" className={TOOLTIP_STYLE}>Открыть сайт в новой вкладке</TooltipContent>
                </Tooltip>
              )}
            </div>
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              <Tooltip>
                ...
                <TooltipContent side="top" className={TOOLTIP_STYLE}>Аналитика</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                ...
                <TooltipContent side="top" className={TOOLTIP_STYLE}>Настройки</TooltipContent>
              </Tooltip>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 py-1.5 px-2 rounded-md bg-muted/20">
            <Tooltip>
              ...
              <TooltipContent side="top" className={TOOLTIP_STYLE}>
                Количество ключевых слов, находящихся на первой странице выдачи (позиции 1–10)
              </TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className={TOOLTIP_STYLE}>
                Средневзвешенная позиция всех отслеживаемых запросов по этому проекту
              </TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className={TOOLTIP_STYLE}>
                Процент вероятности того, что пользователи увидят ваш сайт по ключевым запросам (Visibility Index)
              </TooltipContent>
            </Tooltip>
          </div>
