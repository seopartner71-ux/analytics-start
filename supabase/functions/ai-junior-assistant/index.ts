...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ragBlock = ragChunks.length
      ? ragChunks.map((c, i) => `[Фрагмент ${i + 1}]\n${c.content}`).join("\n\n---\n\n")
      : "(во внутренней базе знаний не найдено релевантного контекста)";

    const fullSystem = `${systemPrompt}

СТРОГИЕ ПРАВИЛА:
- Отвечай ТОЛЬКО на основе данных из разделов «Стандарты компании», «Контекст из внутренней базы знаний», «Статьи базы знаний» и «Задачи пользователя» ниже.
- НИЧЕГО НЕ ВЫДУМЫВАЙ. Если ответа нет в предоставленных данных — честно скажи «В нашей базе знаний нет ответа на этот вопрос, уточни у старшего специалиста».
- НИКОГДА не упоминай названия книг, файлов, PDF или источников. Не пиши «Источник: …», «📄», «согласно книге», «в книге сказано» и т.п. Просто давай ответ.
...
---
## Доступные статьи базы знаний (если релевантно — посоветуй прочитать по названию статьи):
${kbList}
